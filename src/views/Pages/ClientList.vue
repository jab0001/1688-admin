<template>
  <div class="client-list">
    <div class="client-list__wrapper">
      <h2 class="client-list__title">Список пользователей</h2>
      ...
      <tbody>
        ...
        >
          <td
            ...
            v-show="element.visible"
          >
            <span v-if="index === 'long_promo'">{{ client[index]?.name }}</span>
            <span v-else-if="index === 'active'">{{
              !!client[index] ? 'Доставлен' : 'Отклонен'
            }}</span>
            <span v-else-if="index === 'created_at'">{{
              formatDate(client[index])
            }}</span>
    ...
    <ul class="client-list__list">
      <li v-for="page in displayedPages" :key="page" class="client-list__item">
        <button
          @click="getData(page)"
          :class="`client-list__btn ${
            page === currentPage ? 'client-list__btn-active' : ''
          }`"
        >
          {{ page }}
        </button>
...
import ModalAddClient from '@/components/Modal/ModalAddClient.vue';
...
export default {
  ...
  data() {
    return {
      ...
      isNewUserOpen: false,
      tableVisible: {
        username: { visible: true, name: 'ФИО' },
        delivery_status: { visible: true, name: 'Статус' },
        marking: { visible: true, name: 'Маркировка' },
        percent: { visible: true, name: 'Процент' },
        total_sum: { visible: true, name: 'Общая сумма заказа' },
        long_promo: { visible: true, name: 'Промокод' },
        created_at: { visible: true, name: 'Доставлен' },
        phone: { visible: true, name: 'Телефон' },
        email: { visible: true, name: 'Почта' },
      },
      startPagination: 1,
      totalPages: null,
      displayedPages: null,
  ...
  methods: {
    ...mapActions({
      fetchClientsList: 'admin/fetchClientsList',
    }),
    ...
    searchHandler() {
      ...
      this.clientsList = this.getClientsList.filter((item) =>
    ...
    async getData(page) {
      ...
      await this.fetchClientsList(page).then((res) => {
        this.clientsList = this.getClientsList;
    ...
    calculateDisplayedPages() {
      ...
      // Отобразить страницу -1 от текущей (если она больше 1)
      if (this.currentPage > 2) {
        this.displayedPages.push(this.currentPage - 1);
      }

      // Отобразить текущую страницу
      this.displayedPages.push(this.currentPage);

      // Отобразить страницу +1 от текущей (если она меньше последней)
      ...
      this.displayedPages = Array.from(new Set(this.displayedPages)).sort(
        (a, b) => a - b
      );
    },
  },
